...
public class MultiIOprocessing {
	...
	public String copyFiles(File source, File dest) {
		try {
			...
			eUtils.myException(CustomErrorMessage.IOEXCEPTION_COPY,  e.getMessage());
		}
		return "Files are copied";
	}
	...
	public void changeAppjs(String path, ArrayList<String> providername, String what) {
		...
		try {
			...
			eUtils.myException(CustomErrorMessage.IOEXCEPTION_1,  e.getMessage());
		} finally {
			if (out != null)
				out.close();
			try {
				...
				eUtils.myException(CustomErrorMessage.IOEXCEPTION_2,  e.getMessage());
			...
			try {
				...
				eUtils.myException(CustomErrorMessage.IOEXCEPTION_3,  e.getMessage());
			}
		}
	...
	// addd url
	public String changeConfig(String path, String appname, Environment_Json environmentJson, JSONObject hookObj,
			String url) {
		String directory = path + "/src/config.json";  
		Path file = Paths.get(directory);
		JSONObject job = new JSONObject(environmentJson);
...
//			}
		...
		try {
			...
			eUtils.myException(CustomErrorMessage.IOEXCEPTION_4,  e.getMessage());
		} 
		return url;
	}
	...
	public void removeOauthJade(String source, String dest) {
		try {
			...
			eUtils.myException(CustomErrorMessage.IOEXCEPTION_5,  ex.getMessage());
		}
	}
...
//	public JSONObject envoirmentjson(PrimaryProviders primaryProviders,Logger logger){
...
//		}
//		envjson.put("log_provider",logger.getName());
//		envjson.put("host", logger.getHostname());
//		envjson.put("DBName", logger.getDbName());
//		envjson.put("port", logger.getPortNo());
//		envjson.put("username", logger.getUsername());
//		envjson.put("password", logger.getPassword());
//		envjson.put("type", logger.getType());
//		envjson.put("protocol", logger.getLoggerprotocol());
//		envjson.put("password", logger.getPassword());
//		return envjson;
//	}
//	public JSONObject envoirmentjsonlogger(Logger logger){
//		JSONObject envjson = new JSONObject();
//		/*addfor everything*/
//		if(!(logger.getName()==null)){
//			envjson.put("log_provider", logger.getName());
//			envjson.put("host", logger.getHostname());
//			envjson.put("log_provider", logger.getDbName());
//			envjson.put("port", logger.getPortNo());
//			envjson.put("username", logger.getUsername());
//			envjson.put("password", logger.getPassword());
//		}
...
//	}
	public String changeConfig(String dir,String appname){
		...
		try {
			...
			eUtils.myException(CustomErrorMessage.IOEXCEPTION_6,  e.getMessage());
		}
		return directory;
	}
}
