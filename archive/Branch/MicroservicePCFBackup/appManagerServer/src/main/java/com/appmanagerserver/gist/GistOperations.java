 ...
 */
	...
	public AppfactoryResponse readGistContent(String gistFileName) throws IOException {
		...
		try {
			...
			if (conn.getResponseCode() == HttpStatus.UNAUTHORIZED.value()) {
				...
				/*Gson gson = new Gson();
		String content = getGistInstance().getGist(gistFileName).getFiles()
				.get(ApplicationConstants.GIST_STATUS_FILE_NAME).getContent();
		Type collectionType = new TypeToken<AppfactoryResponse>() {
		}.getType();
		AppfactoryResponse gistResponseVO = gson.fromJson(content, collectionType);*/
		return null;
	}
	...
	public String defaultGistContent(String messageid, String input) {
		try {
			...
			LogList.setLoglevel("INFO");
			LogList.setMessage("Staged for deployment");
 ...
 */
	...
	public String uploadTempapp(String gistid,String sourcePath,String destination) {
		try {
			...
			String passWord=Messages.getString("DeploymentStsus.gist.user");
