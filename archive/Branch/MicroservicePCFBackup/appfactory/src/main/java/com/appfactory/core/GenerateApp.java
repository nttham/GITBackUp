...
public class GenerateApp {
	...
	public String baseDirectory()  {
		...
		System.out.println("This is the 1st time");
		File maindirectory=miop.createFolder(Messages.getString("Base_path")+"/appManager", "app");
//		String parentdirectory = System.getProperty("user.dir")+accessdata.getMessageid();
		
		String parentdirectory = maindirectory+ accessdata.getMessageid();
		System.out.println("This is the 2nd time");
		...
		if (!(createdir.exists())) {
			...
			su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
					ApplicationConstants.BASE_DIRECTORY_FAILED, ApplicationConstants.CFURL_FAILED, 102, ApplicationConstants.LOG_LEVEL_ERROR);
			return FAILURE;
		}
		...
		su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
				ApplicationConstants.BASE_DIRECTORY, ApplicationConstants.CFURL, 101,ApplicationConstants.LOG_LEVEL_INFO);
		try {
			return cloneGit();
		} catch (MyException e) {
	...
	private String cloneGit() throws MyException {
		...
		if (returnarr.get(0).equalsIgnoreCase("error")) {
			...
			su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
					ApplicationConstants.CLONE_TEMPLATE_FAILED, ApplicationConstants.CFURL_FAILED, 102, ApplicationConstants.LOG_LEVEL_ERROR);
			return FAILURE;
		}
		...
		su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
				ApplicationConstants.CLONE_TEMPLATE, ApplicationConstants.CFURL, 101, ApplicationConstants.LOG_LEVEL_INFO);
		return generateURL();
	}
	...
	private String generateURL() throws MyException {
		...
		if (accessdata.getModifiedprotocol().equalsIgnoreCase(ApplicationConstants.LOGGER)) {
			...
			su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
					ApplicationConstants.URL_GENERATED, ApplicationConstants.CFURL, 101, ApplicationConstants.LOG_LEVEL_INFO);
			return generateZip();
		}
		miop.changeConfig(
				accessdata.getParentdirectory() + Messages.getString("AppManager.frwdslash")
						+ accessdata.getuIModelJson().getServicename(),
				accessdata.getuIModelJson().getServicename(),
				accessdata.getuIModelJson().getEnvironment_json(), accessdata.getHookObj(),
				accessdata.getRunningapp());
		if (urls.isEmpty()) {
			su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
					ApplicationConstants.URL_GENERATED_FAILED, ApplicationConstants.CFURL_FAILED, 102, ApplicationConstants.LOG_LEVEL_ERROR);
			return FAILURE;
		}
...
//			accessdata.setEnv_json(mip.envoirmentjson(accessdata.getBlueprint().getPrimaryservice().getProviders(),accessdata.getBlueprint().getLogger()));
			...
			String response=bservice.createServiceInstance(token, accessdata.getuIModelJson().getSpaceguid(),accessdata.getuIModelJson().getServiceplanguid(),
					platform, createservice,"", auth,accessdata.getuIModelJson().getServicename(),forwhat);
			return gitPush();
		}
		return pushApp();
	}
	...
	private String pushApp() throws MyException {
		...
		String response = pushtheapp.checkFortheplatform(accessdata.getuIModelJson().getPlatform(),
				...
				accessdata.getMessageid(), accessdata.getuIModelJson().getGistid());
		if (response.contains("failed")) {

			su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
					ApplicationConstants.PLATFORM_UPLOADPP_FAILED, ApplicationConstants.CFURL_FAILED, 102, ApplicationConstants.LOG_LEVEL_ERROR);
			return FAILURE;
		}
		...
		su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
				ApplicationConstants.PLATFORM_UPLOADPP, ApplicationConstants.CFURL, 101, ApplicationConstants.LOG_LEVEL_INFO);
	...
	private String createMicro() throws MyException {
		...
		if (!accessdata.getLogstatus().equalsIgnoreCase(ApplicationConstants.EXTERNAL)) {
			...
			su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
					ApplicationConstants.BIND_APPSERVIVCE, ApplicationConstants.CFURL, 101,ApplicationConstants.LOG_LEVEL_INFO);
		}
	...
	private String configureService() throws MyException {
		...
		for (OtherServices each : otherserv) {
			...
			createinstanceobj.put("service_plan_guid", each.getServiceplanguid());
			...
			try {
				...
				System.out.println("my value= " + response);
				LOG.info("Otherservice instance is created");

				su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
						ApplicationConstants.OTHERSERVICE, ApplicationConstants.CFURL, 101, ApplicationConstants.LOG_LEVEL_INFO);

			} catch (MyException e) {
				su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
						ApplicationConstants.OTHER_SERVICE_ERROR, ApplicationConstants.CFURL_FAILED, 102, ApplicationConstants.LOG_LEVEL_ERROR);
				throw utils.myException(CustomErrorMessage.OTHER_SERVICE_ERROR, e.getLocalizedMessage());
			}

			/* This is for binding the app to otherservice */
			JSONObject inputobj = new JSONObject();
			inputobj.put("service_instance_guid", each.getServiceplanguid());
			...
			try {
				...
				su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
						ApplicationConstants.OTHERSERVICE_BIND, ApplicationConstants.CFURL, 101, ApplicationConstants.LOG_LEVEL_INFO);
			} catch (MyException e) {
				su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
						ApplicationConstants.OTHERSERVICE_BIND_ERROR, ApplicationConstants.CFURL_FAILED, 102, ApplicationConstants.LOG_LEVEL_ERROR);
				throw utils.myException(CustomErrorMessage.OTHER_SERVICE_ERROR, e.getLocalizedMessage());
			}
		}
		return gitPush();
	...
	private String gitPush() {
		...
		if (accessdata.getuIModelJson().getDevlopergiturl().isEmpty()) {
			su.updatestatus(accessdata.getMessageid(), accessdata.getuIModelJson().getGistid(),
		...
		/*if (!(result.equalsIgnoreCase(SUCCESS))) {
			gito.uploadTempapp(accessdata.getBlueprint().getGistid(), copydirzip, destination);
>>>>>>> fc5bc280d365e0fe2e664c190ecfd3847b55b21d
			return FAILURE;
		}*/
