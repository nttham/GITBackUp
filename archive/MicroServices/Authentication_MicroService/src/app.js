...
var file = __dirname + '/public/api-docs/data.json'
delete swaggerDefinition.host;

var services_env =  {"uris": ["localhost:3000"]};//JSON.parse(process.env.VCAP_APPLICATION || "{}");
console.log(services_env.uris[0]);
swaggerDefinition.host = services_env.uris[0];
var host = services_env.uris[0];
var swaggerSpec;
...
app.configure(function () {
    ...
    var options = {
        ...
        apis: ['./providers/oauth/*.js', './hooks/*.js'],
    };
...
// serve swagger
app.get('/auth/swagger/docs', function (req, res) {
    res.setHeader('Content-Type', 'application/json');
    res.send(swaggerSpec);
});

// serve swagger UI
app.get('/auth/swagger/UI', function (req, res) {
    res.setHeader('Content-Type', 'application/json');
    var docUrl = constants.protocol + host + constants.swaggerPath;
    res.redirect(docUrl);
    });
...
if (config && config.configuration && config.configuration.facebook && config.configuration.facebook.clientID && config.configuration.facebook.clientSecret && config.configuration.facebook.scope) {
...
if (config && config.configuration && config.configuration.google && config.configuration.google.clientID && config.configuration.google.clientSecret && config.configuration.google.scope) {
...
if (config && config.configuration && config.configuration.linkedin && config.configuration.linkedin.clientID && config.configuration.linkedin.clientSecret && config.configuration.linkedin.scope) {
...
if (config && config.configuration && config.configuration.twitter && config.configuration.twitter.clientID && config.configuration.twitter.clientSecret) {
...
//LDAP initiation
if (config && config.configuration && config.configuration.ldap && config.configuration.ldap.ldapURL) {
...
if (config && config.configuration && config.configuration.custom && config.configuration.custom.clientID && config.configuration.custom.clientSecret && config.configuration.custom.authorizationURL && config.configuration.custom.tokenURL) {
...
//Hooks initiation
if (config && (config.prehooks || config.posthooks)) {
...
if (config && config.configuration && config.configuration.httprequest) {
...
// port
//here we save the host name to a file for the swaggerUI 
jsonfile.writeFile(file, hostObj, function (err) {
    if (err) {
        console.error(err)
    }
    else {
